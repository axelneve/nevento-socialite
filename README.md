...
## IDP setup (one-time per workspace, done by SuperAdmin)
...
### 2. Assign the `client-sites` app license to the workspace
...
| App | `client-sites` |
| License type | `Gratis` (unlimited sites) |
| Status | `active` |
| Admin seats | 1+ |

### 3. Add the workspace admin to the `client-sites` app

**SuperAdmin → Workspaces → [workspace] → Provisioning → Users → Add to app**

Pick `client-sites` and assign the role `admin`. The admin can now manage their own OAuth clients.

---

## Per-site OAuth client setup (done by workspace admin)

Once the `client-sites` app is assigned, workspace admins self-manage their OAuth clients at:

**Console → [workspace] → Websiteverbindingen** (`/w/{slug}/websiteverbindingen`)

| Action | Description |
|---|---|
| Create | Fill in a name and optionally a redirect URI. Copy the generated `client_id` and `secret` — the secret is shown only once. |
| Update | Edit name or redirect URI at any time. |
| Revoke | Removes the client — the site can no longer authenticate. |

Each connection maps to one website backend (one `NEVENTO_CLIENT_ID` / `NEVENTO_CLIENT_SECRET` pair).
...
**SuperAdmin (one-time per workspace):**
1. Create workspace + assign `client-sites` license (Gratis) in the console
2. Add the workspace admin to the `client-sites` app

**Workspace admin (self-service, per website):**
3. Go to **Console → [workspace] → Websiteverbindingen** and create a new connection
4. Copy the `client_id` and `secret` — secret shown only once

**Site developer (per new website):**
5. Install this package (`composer require eventsolutions/nevento-socialite`)
6. Add the `services.nevento` config block and env vars (`NEVENTO_CLIENT_ID`, `NEVENTO_CLIENT_SECRET`, `NEVENTO_REDIRECT_URL` differ per site)
7. Run the `users` table migration
8. Register the middleware alias and the `login` redirect route
